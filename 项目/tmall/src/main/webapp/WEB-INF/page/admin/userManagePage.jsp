                ...
                success: function (data) {
                    ...
                    if (data.userList.length > 0) {
                        for (var i = 0; i < data.userList.length; i++) {
                            ...
                            //显示用户数据
                            tbody.append("<tr><td><input type='checkbox' class='cbx_select' id='cbx_user_select_" + user_id + "'><label for='cbx_user_select_" + user_id + "'></label></td><td title='" + user_name + "'>" + user_name + "</td><td title='" + user_nickname + "'>" + user_nickname + "</td><td title='" + user_realname + "'>" + user_realname + "</td><td title='" + user_birthday + "'>" + user_birthday + "</td><td title='" + gender + "'>" + gender + "</td><td>" +
                                "<span class='td_special' ><a href='javascript:void(0);' onclick='getChildPage(this)' class='editBtn'>详情</a></span>" +
                                "<span class='td_special' ><a href='javascript:void(0);' onclick='deleteRow(this)'>删除</a>" +
                                "</span></td><td hidden  class='user_id'>" + user_id + "</td></tr>");
                        }
                        //绑定事件
            ...
            <tr>
                ...
                <td>
                    <span class="td_special" >
                        <a href='javascript:void(0)' onclick='getChildPage(this)' class="editBtn">详情</a></span>
...
<script>
    ...
    function deleteRow(obj){
        ...
        var msg = "确定要删除吗？！";
        if (confirm(msg)==true){
            $.get("admin/user/delete/"+id,function(msg){
                if(msg == 1){
                    alert("删除成功！")
                    getData($(this), "admin/user/0/10", dataList);
                }else{
                    alert("该用户存在订单或是存在购物车，不可删除！")
                }
            });
        }
    }
