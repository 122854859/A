                ...
                success: function (data) {
                    ...
                    if (data.productList.length > 0) {
                        for (var i = 0; i < data.productList.length; i++) {
                            ...
                            //显示产品数据
                            tbody.append("<tr><td><input type='checkbox' class='cbx_select' id='cbx_product_select_" + product_id + "'><label for='cbx_product_select_" + product_id + "'></label></td><td title='"+product_name+"'>" + product_name + "</td><td title='"+product_title+"'>" + product_title + "</td><td title='"+product_price+"'>" + product_price + "</td><td title='"+product_sale_price+"'>" + product_sale_price + "</td><td title='"+product_create_date+"'>" + product_create_date + "</td><td>" +
                                "<span class='" + isEnabledClass + "' title='"+isEnabledTitle+"'>"+ isEnabled + "</span>" +
                                "</td><td><span class='td_special'><a href='javascript:void(0);' onclick='getChildPage(this)' class='editBtn'" +
                                " style='color: #337ab7!important;'>修改</a></span>" +
                                "<span class='td_special'><a href='javascript:void(0);' onclick='deleteRow(this)'>删除</a></span>" +
                                "</td><td hidden><span class='product_id'>" + product_id + "</span></td></tr>");
                        }
                        //绑定事件
...
<div class="frm_div text_info">
    ...
    <div class="frm_group_last">
        <input class="frm_btn frm_add" id="btn_product_add" type="button" value="添加" onclick="getChildPage(null)"/>
    ...
    <table class="table_normal" id="table_product_list">
        ...
        <c:forEach items="${requestScope.productList}" var="product">
            <tr>
                ...
                <td><span class="td_special" ><a href="javascript:void(0)" onclick="getChildPage(this)" style="color: #337ab7!important;">修改</a></span>
                    <span class='td_special'><a href='javascript:void(0);' onclick='deleteRow(this)'>删除</a></span>
                </td>
...
<script>
    ...
    function deleteRow(obj){
        var id = $(obj).parents("tr").find(".product_id").text();
        console.log(id)
        var msg = "确定要删除吗？！";
        if (confirm(msg)==true){
            $.get("admin/product/delete/"+id,function(msg){
                if(msg == 1){
                    alert("删除成功！")
                    getData($(this), "admin/product/0/10", null);
                }else{
                    alert("该商品已有用户购买，不可删除！")
                }
            });
        }
    }
